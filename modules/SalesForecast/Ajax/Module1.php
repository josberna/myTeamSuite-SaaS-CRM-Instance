...
$month = array ('01'=>"Jan", 
   ...
   if($role != 'H5')
   {
	   ...
	   $response .='<td style="border: 1px solid black;">'.$GLOBALS['subtract'].' $</td>';
   }
   $response .= '</tr>';
...
function GetTotalOfMonth($month,$year) 
{
	...
	$result=$adb->query("select quota from vtiger_salesforecasttype_month where period = '".$last_date."'");
	...
	if($no_rows > 0 )
	{
		...
		if($acc_row['quota'] != '')
		{
			...
			$res = $adb->query("select sum(quota) as quota	from vtiger_salesforecast_quota where start_date ='".$date."' and end_date  = '".$last_date."'");
...
function GetMonthyDetail()
{
	...
	$role=$current_user->column_fields['roleid'];
	if($current_user->is_admin == 'on' || $role == 'H1' || $role == 'H2' || $role == 'H3')// 
	{
	...
	{
		$query="select ".$cols." from vtiger_users vu, vtiger_user2role vr ,vtiger_role vrole
				where vu.id = vr.userid and vrole.roleid = vr.roleid and vr.roleid = 'H5' and (vr.roleid = 'H4' and vr.userid=".$current_user->id.")";
	}
...
{
	global $adb,$current_user;
	$result =  $adb->pquery("select * from vtiger_salesforecasttype_month where period = ? ",array($_REQUEST['period']));
	$no_acc_rows=$adb->num_rows($result);
	
	if($no_acc_rows == 0)
	{
		$adb->pquery("insert into vtiger_salesforecasttype_month(period,quota) values(?,?)",array($_REQUEST['period'],$_REQUEST['quota']));
	}
	else
	{
		$adb->pquery("update vtiger_salesforecasttype_month set quota=?	where period=? ",array($_REQUEST['quota'],$_REQUEST['period']));
	}
	return GetMainView($_REQUEST['sdate'], $_REQUEST['edate']);
}
?>