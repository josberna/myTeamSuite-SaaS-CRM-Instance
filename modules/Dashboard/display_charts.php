...
//Query for tickets by contact
$sales_by_opportunity="select vtiger_potential.* from vtiger_potential inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_potential.potentialid where vtiger_crmentity.deleted=0";
...
$graph_array = Array(
	  ...
	  "salesbyopportunity"=>$mod_strings['salesbyopportunity']
  );
  	if(isset($_REQUEST['from_page']) && $_REQUEST['from_page'] == 'HomePage' && $type=="salesbyopportunity")
	{
		include_once('modules/Dashboard/pie_custom_chart_home.php');
        return get_chart();
  	}
	...
	else
	{
					...
					if($type == "salesbyopportunity")
                    {
						//echo '$$';
						$IS_FUNNEL='azeem';
						//$IS_FUNNEL=true;
                    	$graph_by="salesbyopportunity";
                    	$graph_title= $mod_strings['salesbyopportunity'];
                    	$module="Potential";
                    	$where="";
						$query =$sales_by_opportunity;
                    	//$query=getDashboardQuery($sales_by_opportunity,$module);
                    	//$html .= get_graph_by_type($graph_by,$graph_title,$module,$where,$query);
						include_once('modules/Dashboard/pie_custom_chart.php');
                        echo get_chart();
                    }
                    else if($profileTabsPermission[getTabid("Leads")] == 0 && ($type == "leadsource") && (getFieldVisibilityPermission('Leads',$user_id,'leadsource') == "0"))
                    {
                    	$graph_by="leadsource";
                    	$graph_title= $mod_strings['leadsource'];
                    	$module="Leads";
