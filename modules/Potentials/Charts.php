...
class jpgraph {
	...
	function outcome_by_month($date_start='1971-10-15', $date_end='2071-10-15', $user_id=array('1'), $cache_file_name='a_file', $refresh=false,$width=900,$height=500){
		...
		function outcome_by_month1($date_start='1971-10-15', $date_end='2071-10-15', $user_id=array('1'), $cache_file_name='a_file', $refresh=false,$width=900,$height=500){
		global $log;
		$log->debug("Entering outcome_by_month(".$date_start.",". $date_end.",". $user_id.") method ...");
		global $app_strings,$lang_crm, $app_list_strings, $current_module_strings,$current_user, $log, $charset, $tmp_dir;
		global $theme;
...
function pipeline_by_sales_stage($datax=array('foo','bar'), $date_start='2071-10-15', $date_end='2071-10-15', $user_id=array('1'), $cache_file_name='a_file', $refresh=false,$width=900,$height=500){
			...
			/* Draw the scale  */
			$myPicture->setGraphArea(50,30,($width-20),($height -110));
			...
			/* Draw the chart */
			$settings = array("Floating0Serie"=>"Floating 0","Draw0Line"=>TRUE,"Gradient"=>TRUE,"DisplayPos"=>LABEL_POS_INSIDE,"DisplayValues"=>TRUE,"DisplayShadow"=>TRUE,"Surrounding"=>10,"OverrideColors"=>$Palette,"DisplayR"=>255,"DisplayG"=>255,"DisplayB"=>255);
			$myPicture->drawBarChart($settings);
			/* Write the chart legend */
			$titlestr = $current_module_strings['LBL_TOTAL_PIPELINE'].$current_user->currency_symbol.$total.$app_strings['LBL_THOUSANDS_SYMBOL'];
			$myPicture->drawText(50,20,$titlestr,array("FontSize"=>12)); 
			//$myPicture->drawLegend(580,12,array("Style"=>LEGEND_NOBORDER,"Mode"=>LEGEND_HORIZONTAL)); 
			/* Left orange surrounding box with an alpha factor of 30% */
			$RectangleSettings = array("R"=>255,"G"=>255,"B"=>255,"Alpha"=>100);
			$value = ($width/2) + ($width/4);
			$myPicture->drawRoundedFilledRectangle($value,10,$value-200,20,0,$RectangleSettings);
			$myPicture->Render($cache_file_name);
		return '<img src="'.$cache_file_name.'"  />';
	}
	
	function pipeline_by_sales_stage1($datax=array('foo','bar'), $date_start='2071-10-15', $date_end='2071-10-15', $user_id=array('1'), $cache_file_name='a_file', $refresh=false,$width=900,$height=500){
		global $log,$current_user;
		$log->debug("Entering pipeline_by_sales_stage(".$datax.",".$date_start.",".$date_end.",".$user_id.",".$cache_file_name.",".$refresh.") method ...");
		global $app_strings,$lang_crm, $current_module_strings, $charset, $tmp_dir;
		global $theme;
	 ...
	 * Contributor(s): ______________________________________..
	 */
	 	function pipeline_by_lead_source1($legends=array('foo','bar'), $user_id=array('1'), $cache_file_name='a_file', $refresh=true,$width=900,$height=500){
		global $log,$current_user;
...
$log->debug("Exiting pipeline_by_lead_source method ...");
	...
	function pipeline_by_lead_source($legends=array('foo','bar'), $user_id=array('1'), $cache_file_name='a_file', $refresh=true,$width=900,$height=500){
		...
		// ADD POINTS
		$MyData->addPoints($points,"ScoreA");  
		$MyData->setSerieDescription("ScoreA","Application A");
		// ADD LABEL
		...
		$PieChart->draw3DPie($width/3,$height/3,array("Radius"=>$width/4,"DataGapAngle"=>12,"DataGapRadius"=>10,"Border"=>TRUE));
		
		/* Write the legend box */ 
		$myPicture->setFontProperties(array("FontName"=>"include/pchart/fonts/verdana.ttf","FontSize"=>10,"R"=>0,"G"=>0,"B"=>0));
		$PieChart->drawPieLegend($width/1.5,$height/4.9,array("Style"=>LEGEND_NOBORDER,"Mode"=>LEGEND_VERTICAL));
